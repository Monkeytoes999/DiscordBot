...
bot.on('message', function (user, userID, channelID, message, evt) {
	...
	for (var i = 0; i < curses.length; i++){
		if (true || cussIndexes.length > 0 && !allowCuss && !channel.nsfw || message.includes('A$$H0L3')){
			if (true || nonWordCurses[i] != 'no' || message.includes('A$$H0L3') || cussmessage.substring(0, (curses[i].length)) == curses[i] || cussmessage.includes(' ' + curses[i])){
				bot.deleteMessage({
					channelID: channelID,
					messageID: prevEvtID
				});
