	...
	if (true){
		...
		if ((day != prevDay && scDay != 'oof') || message == "gcd.gcd.gcd...gcD."){
			...
			if(scDay.toUpperCase() != 'NONE'){
				let scAnnounce = 0;
				let scAnnArr = ['458809225120972800','486985623161274378','336507246227881984','393586279964475393','120545011392577537','250331063337877504'];
				let scAnnMArr = ['jon', 'bert', 'cy', 'g', 'civ', 'voosh'];
				let scOof = 0;
			...
			if (!(bot.users.get(userID).bot)){
				...
				switch(cmd) {
					case 'announce':
						if (userID == gID){
							let scAnnounce = 0;
							let scAnnArr = ['458809225120972800','486985623161274378','336507246227881984','393586279964475393','399366382799814656','120545011392577537','250331063337877504'];
							let scAnnMArr = ['jon', 'bert', 'cy', 'g', 'er', 'civ', 'voosh'];
							let scOof = 0;
							while (scAnnounce < scAnnArr.length){
